...
* If on Windows:
  ...
  * run
    * `Skia\build`
    * `Demo\build`
	* Start the server using below command from the root of the project:
			python -m http.server
			And then open browser and open the html file created inside the Demo build folder in the browser using http://localhost:8000/Demo/out/release/	
